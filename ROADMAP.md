...
2. CLI decomposition by responsibility:
   Split `src/cli/index.ts` into presentation, wiring, and dispatch across three behavior-preserving slices. First move the formatter functions into `src/cli/present/` grouped by domain (writing, sessions, explain, knowledge, memory, browser, preview, plus a shared helper file), extract the `run` and `resume` command bodies into `src/cli/commands/`, move model-client and approval-handler assembly into `src/cli/wiring.ts` in the same slice so command bodies never import back into the dispatcher, and move the interactive terminal output controller into `src/cli/terminal.ts`, keeping per-workflow typed session entry points per ADR 0025. Second, sink the provider-for-model mapping and the model runnability check — including its exact user-facing error and preview texts — into `src/models/routing.ts` with a type-only import of `ForgeletConfig`, so adding a provider never edits CLI files; `routeModel` stays in `src/config` because the kernel also routes through it. Third, sink the Writing Project run rules — the `--allow-read` conflict check, member and head validation, and continuation resolution — into one composed `src/writingProjects` entry point, preserving today's check order, so Project Manifest and Continuation Head rules live with the domain and later surfaces such as the local review UI cannot bypass or reorder them. This split is also the precursor to Background Sessions: once wiring leaves the CLI, a Session stops assuming an interactive terminal.
   Accept each slice when all existing tests pass unchanged and `runCli` stdout/stderr shapes and error messages stay identical. The first slice adds no new tests; the second adds unit tests for the routing contract; the third adds unit tests for the composed Writing Project rules and their order. Accept the final slice when `src/cli/index.ts` contains only command dispatch: one-call cases, the entry point, and re-exports that keep existing importers working.
